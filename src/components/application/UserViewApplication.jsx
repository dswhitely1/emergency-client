...
function UserViewApplication() {
    ...
    if (!profile.id) {
        return (
            <>
                <Typography variant='h2' className={clsx(classes.title, classes.appSpacing)}>No Application on
                    File...</Typography>
                <Typography paragraph className={clsx(classes.title, classes.appSpacing)}>Click {<Link
                    to='/dashboard/profile'>Here</Link>} to get
                    started</Typography>
            </>)
    }

    return (
        <ViewApplication values={profile} employment={employment} education={education} reference={reference} />
    )
