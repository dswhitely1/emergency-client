...
function AdminSelectProfile() {
    const {isLoading, profiles, selectedProfile: {profile, employment, education, reference}} = useSelector(state => state.admin);
    ...
    useEffect(() => {
        setUserEmployment(employment.map(item => {
            return {
                ...item,
    ...
    return (
        <>
            <Container maxWidth='lg'>
                <Paper className={classes.root}>
                    ...
                    <FormControl className={classes.formControl}>
                        ...
                        <Select labelId='graduate-select-label' id='graduate-select' value={selectProfile}
                                onChange={e => setSelectProfile(e.target.value)}>
                            {profiles.map(profile => <MenuItem key={profile.id}
                                                               value={profile.userId}>{`${profile.firstName} ${profile.lastName}`}</MenuItem>)}
                        </Select>
                    </FormControl>
                </Paper>
            </Container>
            { isLoading ? <Spinner /> :
            profile.id &&
            <ViewApplication values={profile} employment={userEmployment} reference={reference}
                             education={education}/>}
        </>
    )
}
