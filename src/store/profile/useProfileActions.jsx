...
export const useProfileActions = () => {
  ...
  const getProfile = useCallback(
    (token) => {
      ...
      axios(token)
        ...
        .catch((err) =>
          dispatch({ type: types.GET_PROFILE_FAILURE, payload: err.response }),
        );
    },
    [dispatch],
  );
  ...
  const addProfile = useCallback(
    (token, newProfile) => {
      ...
      axios(token)
        ...
        .catch((err) =>
          dispatch({ type: types.ADD_PROFILE_FAILURE, payload: err.response }),
        );
    },
    [dispatch],
  );
  ...
  const updateProfile = useCallback(
    (token, updatedProfile) => {
      dispatch({ type: types.UPDATE_PROFILE_START });
      axios(token)
        .put(`/profiles/${updatedProfile.id}`, updatedProfile)
        .then((res) =>
          dispatch({
  ...
  const deleteProfile = useCallback(
    (token, id) => {
      ...
      axios(token)
        .delete(`/profiles/${id}`)
        .then(() => dispatch({ type: types.DELETE_PROFILE_SUCCESS }))
        .catch((err) =>
          dispatch({
